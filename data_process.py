...
def process_data(pra_now_dict, pra_start_ind, pra_end_ind, pra_observed_last):
    ...
    visible_object_value = np.array(
        list(pra_now_dict[pra_observed_last].values()))  ## the mean x,y at 'pra_observed_last' frame
    ...
    now_all_object_id = set([val for x in range(pra_start_ind, pra_end_ind) for val in
                             pra_now_dict[x].keys()])  ###all_object_id in sequence
    non_visible_object_id_list = list(now_all_object_id - set(
        visible_object_id_list))  ##all_object_id in sequence time subtract object_id in last observed frame
    ...
    adjacency_list = np.hstack(
        [adjacency_matrix_seq[0]] + [ones_matrix] + (adjacency_matrix_seq.shape[0] - 2) * [zeros_matrix])
    ...
    for frame_id_graph in range(1, adjacency_matrix_seq.shape[0] - 1):
        ...
        temp_graph = np.hstack(
            (frame_id_graph - 1) * [zeros_matrix] + [ones_matrix] + [adjacency_matrix_seq[frame_id_graph]] + [
                ones_matrix] + (adjacency_matrix_seq.shape[0] - frame_id_graph - 2) * [zeros_matrix])
    ...
    last_block = np.hstack(
        ((adjacency_matrix_seq.shape[0] - 2) * [zeros_matrix] + [ones_matrix] + [adjacency_matrix_seq[-1]]))
