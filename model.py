...
class Model(nn.Module):
    ...
    def forward(self, pra_x, pra_A, pra_A_big, pra_pred_length, pra_teacher_forcing_ratio=0, pra_teacher_location=None):
        ...
        now_predict_human_TF = self.transformer_model(src=graph_conv_feature, tgt=pra_teacher_location,
                                                      tgt_mask=tgt_mask)
        now_predict_human_TF = self.reshape_from_lstm(now_predict_human_TF)

        now_predict_bike_TF = self.transformer_model(src=graph_conv_feature, tgt=pra_teacher_location,
                                                     tgt_mask=tgt_mask)
